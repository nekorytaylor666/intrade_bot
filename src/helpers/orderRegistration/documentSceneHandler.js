...
documentStepHandler.action('next', (ctx) => {
    ctx.editMessageText(`Вы пропустили прикрепление документов. Заказчики будет ориентироваться только на описание вашего заказа, что может повлиять на качество его выполнения.`, Extra.HTML().markup(m => m.inlineKeyboard([])));
    ctx.reply('Нажмите кнопку "Далее", чтобы перейти к следующему этапу или "Назад", чтобы прикрепить документ.', Markup
        .keyboard([
            ['Далее'],
            ['Назад'],
        ])
...
documentStepHandler.action('back', (ctx) => {
    ctx.editMessageText(`Вы вернулись на этап 1/3`, Extra.HTML().markup(m => m.inlineKeyboard([])));
...
documentStepHandler.use((ctx) => ctx.reply('Нажмите на одну из кнопок или отправьте документ'));
